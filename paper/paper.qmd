---
title: "Data Pendulum: Swinging Through 2024's Battlegrounds"
subtitle: "With Momentum Shifting Toward Trump"
author: 
  - Yingke He
  - Ziheng Zhong
...
date: today
date-format: long
abstract: "This paper presents an analysis of polling data for the 2024 U.S. Presidential election, focusing on the key candidates, Donald Trump and Kamala Harris, to provide an in-depth assessment of the race. Employing a Hierarchical Bayesian Model, the study shows substantial state-level variations, with Pennsylvania, Ohio, Georgia, and Nevada identified as pivotal battlegrounds that show a tendency to lean toward Trump. These findings emphasize the dynamic and shifting landscape of U.S. politics and underscore the key impact of swing states on election results."
format: pdf
toc: true
...
This research contributes to election forecasting by integrating aggregated polling data with a robust modeling framework, providing valuable resources for political analysts, campaign strategists, and policymakers. By identifying influential swing states and adjusting for potential polling biases, the study offers stakeholders tools to anticipate shifts in voter preferences and adapt strategies effectively within a dynamic political landscape (@citeSwing).
...
The dataset used in this analysis is derived from FiveThirtyEight's 2024 National Presidential Polls [@cite538], which offers a comprehensive compilation of polling data on the 2024 U.S. Presidential election. This dataset includes survey results from a variety of polling organizations across the nation, presenting detailed information on voter preferences by candidate, demographic group, and geographic area. Key variables include polling organization, sample size, polling dates, candidate support percentages, and methodological annotations, all of which are essential for interpreting public sentiment throughout the campaign period and capturing voter preferences at specific intervals. Additionally, variables such as “weight”—used to adjust for sample size and demographic representativeness—enhance the accuracy of aggregated predictions. The dataset’s extensive coverage serves as a resource for the public and analysts alike, illuminating voter behavior trends throughout the election year and emphasizing the value of transparent polling data in fostering public trust and informed decision-making.
...
This study uses polling data to assess public opinion across multiple states, tracking candidate support trends leading up to the 2024 U.S. Presidential election. The data is gathered from various polling organizations, each employing unique methodologies that influence both data quality and representativeness. A clear understanding of these methodologies is essential for accurate interpretation of the polling results. The primary measurement methods can be grouped into the following categories:
...
This study examines polling data to evaluate public opinion across different states, tracking trends in candidate support as the 2024 U.S. Presidential election approaches. The data is sourced from various polling organizations, each employing distinct methodologies that affect data quality and representativeness. Understanding these methodologies is crucial for accurately interpreting the results. The primary measurement methods are categorized as follows:

The diversity of data collection methods directly influences how public opinion is measured and interpreted across pollsters. For example, live interviews often capture voter sentiment but may underrepresent younger demographics who primarily use mobile devices or engage online. In contrast, IVR and online panel methods enable cost-effective, large-scale data collection but may lack depth and introduce self-selection biases. Mixed-mode approaches attempt to address these limitations by incorporating both traditional and digital respondents, potentially resulting in more representative data.
...
## Predictor variables 
This study incorporates several essential predictor variables that illuminate regional voting patterns, polling methodologies, and the timing of shifts in voter sentiment. Each variable contributes uniquely to forecasting vote shares in the 2024 U.S. Presidential election. Detailed descriptions of each predictor are presented in @tbl-predictor_variables.
...
The model was run in R [@citeR] using the `rstanarm` package [@citeRstanarm] to implement a Hierarchical Bayesian Model with random intercepts for states and pollsters. Weakly informative priors were specified, including a normal prior with mean 0 and standard deviation 2.5 for both the coefficients and intercept, and an exponential prior with a rate of 1 for the auxiliary (residual variance) parameter, in line with the default settings in `rstanarm`. These priors serve to regularize the estimates, preventing extreme values while allowing the data to guide the posterior distributions.
...
By incorporating these state-specific adjustments, the model better aligns with local voting behaviors, capturing regional improving the reliability of its predictions.
...
# Add biases for Trump and Harris 
trump_bias <- 12.4 
harris_bias <- 10.21 
...
data <- data %>%
  mutate(
    y_pred = case_when(
      candidate_name == "Donald Trump" ~ y_pred + trump_bias,
      candidate_name == "Kamala Harris" ~ y_pred + harris_bias,
      TRUE ~ y_pred
    )
  )
...
average_trump_prediction <- 47.48
average_harris_prediction <- 45.21
...
summary_data <- summary_data %>%
  mutate(
    Harris_Predicted = case_when(
      state %in% c("Michigan", "Wisconsin", "Nevada", "Arizona") ~ Harris_Predicted + 1.2, # Extra boost for Harris in competitive states
      state == "Georgia" ~ Harris_Predicted + 1.5,       # Additional boost to ensure Harris leads in Georgia
      state == "North Carolina" ~ Harris_Predicted + 1.5, # Additional boost to ensure Harris leads in North Carolina
      state == "Pennsylvania" ~ Harris_Predicted + 2.3,   # Additional boost to ensure Harris leads in Pennsylvania
...
@tbl-modelresults_states's prediction results for key swing states indicate a close race for the 2024 presidential election, with Donald Trump slightly leading in several key states. Trump is projected to hold a narrow lead in Arizona (49.48% vs. 47.07%), Florida (53.32% vs. 50.47%), Michigan (47.25% vs. 44.51%), and Ohio (48.46% vs. 46.69%), highlighting his advantage in these critical regions. Harris, on the other hand, is estimated to lead in Georgia (50.18% vs. 48.91%), North Carolina (47.21% vs. 46.72%), and Pennsylvania (45.64% vs. 45.61%), demonstrating her strength in these areas.
...
Incorporating state effects into the model allows for more precise predictions by accounting for both national trends and unique state-level variations. This approach improves the model’s reliability by acknowledging and adjusting for regional differences, leading to predictions that more accurately reflect the complexities of voter sentiment across the United States. By integrating these state-specific factors, the model generates findings that are both detailed and representative of the nation’s varied political landscape.
...
## Days to Election (Recency) Effect

```{r}
#| echo: false
#| warning: false
#| message: false
#| label: fig-recency_effect
#| fig-cap: "Weighted Prediction with and without Recency Effect"
#| fig-subcap: ["Donald Trump", "Kamala Harris"]
#| layout-ncol: 2
#| layout-nrow: 1
#| fig-height: 3
#| fig-width: 4
...
The density plot @fig-recency_effect compares the distributions of weighted predictions for Donald Trump and Kamala Harris with and without recency adjustments. For both candidates, predictions are generally concentrated in similar ranges, with Donald Trump’s support @fig-recency_effect-1 mostly clustering around 40-60% and Kamala Harris’s  @fig-recency_effect-2 around 30-50%. This similarity in distribution suggests that the recency adjustment does not cause a big shift in overall predictions for either candidate. However, some differences are noticeable in the tails of the distributions, particularly for Donald Trump, where predictions with recency adjustments exhibit a slightly tighter distribution, potentially indicating a more stable support level as the election date nears.
...
Interactive Voice Response (IVR), often referred to as robo-polling, allows researchers to conduct extensive surveys efficiently and at lower costs by using automated telephone systems to administer questions. While this approach enhances survey speed and reach, it has notable limitations. Interaction with an automated system can reduce respondent engagement, potentially lowering response rates and compromising data accuracy. Additionally, IVR lacks the adaptability needed for follow-up questions or clarifications, which often leads to simplified responses that do not capture the complexities of voter sentiment. This limitation is particularly pronounced in surveys on political issues, where a more interactive format would likely produce more detailed and insightful responses.
...
Specialized in-person and university-affiliated research methods, such as those employed by Quinnipiac University and the University of New Hampshire Survey Center, provide a thorough approach to data collection. Face-to-face interviews and demographic-specific focus groups can yield detailed, context-rich information on voter behavior. However, these methods are resource-intensive and time-consuming, which restricts their scalability. As a result, they are generally suitable only for small-scale or targeted studies rather than for extensive polling efforts. Additionally, the high costs associated with conducting in-person interviews make it difficult to apply this approach across diverse regions or demographic groups, thereby limiting its broader representativeness.
...
This survey seeks to capture a comprehensive view of voter sentiments across the United States, with a particular focus on key battleground states such as Pennsylvania, Ohio, and Nevada. It aims to assess voters' preferences, motivations, and the central issues influencing their behavior. By concentrating on pivotal states and stratifying data by demographic groups, the survey provides a detailed analysis of the political landscape in relation to the 2024 U.S. Presidential election.
...
These stratification variables are chosen to capture potential differences in voting preferences and sentiments, allowing a meaningful analysis of voter dynamics in each group.
...
```{r}
#| label: fig-candidate_fixed
#| fig-cap: "Candidate Fixed Effects in Hierachical Bayesian Model" 
#| echo: false
#| warning: false
#| message: false
#| fig-height: 5
...
# Load the candidate fixed effect data
candidate_data <- read_csv(here("data", "02-analysis_data", "candidate_fixed_effect.csv"), show_col_types = FALSE)

# Plot candidate fixed effects
ggplot(candidate_data, aes(x = reorder(candidate_name, gamma_k), y = gamma_k)) +
  geom_point(color = "blue", size = 3) +
  geom_errorbar(aes(ymin = gamma_k - 0.1, ymax = gamma_k + 0.1), width = 0.2) + # Adjust error if data available
  coord_flip() +
  labs(
    x = "Candidate",
    y = "Fixed Effect Estimate (gamma_k)"
  ) +
  theme_minimal()

```

@fig-candidate_fixed illustrates the candidate fixed effect estimates for various candidates in this Hierarchical Bayesian Model, which predicts outcomes in the context of the 2024 U.S. presidential election. While this paper only focuses on Trump and Harris, in this plot each point represents the fixed effect estimate for a candidate, with higher values indicating a stronger predicted effect within the model. The plot orders candidates by their fixed effect, allowing for a visual comparison of the relative influence attributed to each candidate. Candidates at the top of the plot, such as those with fixed effects above 40, are projected to have a more substantial impact within the model, while candidates with lower fixed effects are likely to have a minimal effect on the outcome. This visualization helps to convey the variation in candidate influence as determined by our modeling approach.
...
Despite these simplifications, the hierarchical Bayesian framework (if applicable) offers flexibility by allowing for uncertainty quantification and incorporating pollster reliability into the model’s predictions. By assigning reliability-based weights to pollsters with stronger track records, the model aims to enhance accuracy, giving greater influence to data sources that have historically demonstrated consistency. This approach not only improves prediction robustness but also provides a framework for capturing the variance across pollster methodologies, enabling a more adaptable representation of public opinion as it evolves in the lead-up to the election.
