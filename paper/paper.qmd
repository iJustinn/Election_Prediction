...
date: today
date-format: long
abstract: "This paper analyzes polling data for the 2024 U.S. Presidential election, examining key candidates, notably Donald Trump and Kamala Harris, to gain a comprehensive perspective on the race. A Hierarchical Bayesian Model is applied in this study, and the findings reveal significant variations across states, with Pennsylvania, Ohio, Georgia, and Nevada emerging as particularly influential battlegrounds that ultimately tilt in favor of Trump. By highlighting these trends, this research underscores the dynamic and evolving nature of the U.S. political landscape and the crucial role that swing states play in determining election outcomes."
format: pdf
toc: true
...
In this study, polling data shows the public opinion across different states, measuring support for candidates ahead of the 2024 U.S. Presidential election. The data originates from various polling organizations, each having distinct methodologies that influence data quality and representativeness. Therefore, understanding these methodologies is crucial for interpreting polling data accurately. Below shows the primary categories of measurement methods include:
...
To predict the election vote share for each candidate across states, this study employs a Hierarchical Bayesian Model. This model is designed to capture variations in voter support at the state level while accounting for differences introduced by pollsters and specific poll attributes. By structuring the model hierarchically, it accommodates state-level effects, ensuring that local influences on candidate preference are reflected in the predictions. Additionally, the model incorporates poll-specific factors—such as poll sample size, timing, and reliability scores—to enhance its sensitivity to the varying quality and timing of polling data. This comprehensive approach allows the model to generate robust estimates of expected vote shares, providing a thorough view of likely election outcomes.
...
We run the model in R [@citeR] using the `rstanarm` package of @rstanarm. We use the default priors from `rstanarm`.[To be updated...]
...
5. Adjustment of weighted predictions by accounting for recency $( \theta \cdot \mathrm{days\_to\_election}_{ijk} )$
...
state_effect <- read_csv(here("data", "02-analysis_data", "state effect.csv"))

# Check the data structure
glimpse(state_effect)
...
The inclusion of state effects in the model allows for more accurate predictions that account for both national trends and state-specific deviations. This approach enhances the reliability of the model by recognizing and adjusting for regional variations, leading to predictions that more accurately reflect the complexities of voter sentiment across the United States. By accommodating these state-level effects, the model can better provide insights that are both comprehensive and reflective of the current diverse political dynamics.
...
pollster_effects <- read_csv(here("data", "02-analysis_data", "pollster effect.csv"))

# 1. Histogram of Pollster Effects
ggplot(pollster_effects, aes(x = pollster_effect)) +
  geom_histogram(bins = 10, fill = "steelblue", color = "black") +
  labs(
    title = "Distribution of Pollster Effects",
    x = "Pollster Effect",
...
plot_trump <- ggplot(trump_data, aes(x = source, y = weighted_pct, fill = source)) +
...
plot_harris <- ggplot(harris_data, aes(x = source, y = weighted_pct, fill = source)) +
  geom_violin(trim = FALSE) +
  geom_boxplot(width = 0.1, color = "black", fill = "white", outlier.shape = NA) +
...
grid.arrange(plot_trump, plot_harris, ncol = 2)
...
The recency adjustment in the model gives more weight to polls conducted closer to the election, which can enhance the model’s responsiveness to recent changes in voter sentiment. This adjustment is crucial if recent events have significantly impacted public opinion, as it allows the model to prioritize recent data. In this case, the plot suggests that while recency adjustments provide added precision by refining the distribution, they do not drastically alter the predictions. This alignment between the original and recency-adjusted predictions may indicate steady voter sentiment, with minimal late shifts influencing the overall distribution as the election date approaches.
...
The primary data source for this study is the national polling data on the 2024 U.S. Presidential election provided by FiveThirtyEight. This dataset compiles polling results from a variety of polling organizations, each employing distinct methodologies, sample sizes, and timing intervals. By consolidating polls from multiple sources, FiveThirtyEight’s dataset offers a comprehensive view of public opinion trends across the nation, capturing fluctuations in candidate support as the election approaches. This centralized dataset also includes detailed information on each pollster's historical performance and reliability, enabling a more nuanced understanding of poll accuracy and biases.
